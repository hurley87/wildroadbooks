...
export function TeacherInterface() {
  ...
  const { messages, sendMessage, status, error, setMessages, stop } = useChat({
    transport: new DefaultChatTransport({ api: '/api/teacher' }),
  });
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-300px)] max-h-[800px] border border-border rounded-lg overflow-hidden bg-background shadow-lg gilt-edge">
      {/* Header with New Chat button */}
      {messages.length > 0 && (
        <div className="flex items-center justify-between px-6 py-3 border-b border-border bg-muted/20">
          <div className="flex items-center gap-2">
            <SparklesIcon size={16} className="text-primary" />
            <span className="text-sm font-medium text-foreground">Analytics Assistant</span>
          </div>
          <Button
            variant="ghost"
            size="sm"
          ...
          <div className="flex flex-col items-center justify-center h-full px-6 py-12 relative">
            ...
            <div className="relative z-10 text-center max-w-2xl animate-fade-up">
              ...
              <p className="text-muted-foreground mb-8 text-base leading-relaxed">
                Ask me about student comprehension, common misconceptions, response times, 
                and learning patterns. I can search through student responses semantically 
                and provide insights to help improve instruction.
              </p>
                ...
                <div className="flex w-full max-w-3xl mx-auto gap-4 items-center justify-between">
                  <div className="flex items-center gap-4 flex-1">
                    ...
                    <div className="flex items-center gap-2 text-muted-foreground">
                      ...
                      <span className="text-sm">Analyzing student responses...</span>
                    </div>
                  </div>
                  <Button
                    variant="ghost"
